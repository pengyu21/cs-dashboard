...
class GangnamUnniChannel(BaseChannel):
    ...
    #    처럼 고객 취소 사유가 이 형식으로 남는데, 그 줄까지 지우면 메모칸이 빈
    #    것으로 읽혀 취소된 건이 미확인 목록에 계속 떠 있었다(2026-09 실측).
    #    → 메모칸에 이 기록이 남아 있으면 '확인된 건' 으로 본다.
    #    희망일·[선택한 옵션] 은 신규 행에도 항상 자동으로 붙으므로 그대로 걷어낸다
    #    (안 걷어내면 전 행이 '처리됨' 이 되어 신규가 0건이 된다 — 위 주석 참고).
    ...
    def _human_memo(self, td) -> str:
        """메모칸(td#8)에서 '모든 행에 늘 붙는 자동입력'만 걷어내고 남은 내용을 돌려준다.

        걷어내는 것(신규 행에도 항상 들어있어 판정에 쓸 수 없는 값):
          · '희망일1: …' 같은 희망 예약일 줄
        ...
        남기는 것:
          · CS가 직접 쓴 메모
          · '[예약취소] … (2026.09.22 23:34 - 시스템 기록)' 같은 상태 변경 기록.
            이건 실제로 무슨 일이 있었을 때만 찍히므로(신규 행엔 없다) 남아 있으면
            이미 확인·정리된 건으로 본다.
