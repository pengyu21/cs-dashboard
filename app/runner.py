...
안전장치(총정리):
  ...
  3) 닫힌 탭 자동 재생성       7) 크롬 오류 화면('앗, 이런!')이면 자동 재접속
  4) 페이지 로드 타임아웃      8) 오래 쓴 탭은 새 탭으로 교체(메모리 부족 예방)

  ※ 5)는 상주 브라우저(사람이 띄워둔 창)일 때 건너뛴다 → 그 경우 8)이 메모리를
    정리한다(BrowserHub.TAB_RECYCLE_AFTER_SEC).
